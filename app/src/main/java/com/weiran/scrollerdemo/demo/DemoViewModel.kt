...
class DemoViewModel : ViewModel() {
    val topics = provideTopics()
    val taskItems = provideTaskItems()


    val topicsList = provideTopic()
    val selectedTopicWithContent = MutableStateFlow(topicsList.first())

    val selectedTopic = mutableStateOf(topics.first())
}

fun provideTopic(): List<TopicWithContent> {
    return provideTopics().map {
        TopicWithContent(
            topic = it.name,
            content = provideTaskItems().filter { tasks ->
                tasks.topicId == it.id
            }
        )
    }
...
private fun provideTopics() = listOf(
...
private fun provideTaskItems() = listOf(
    TaskItem(
        id = 1,
        topicId = 1,
        content = "Content 1.1"
